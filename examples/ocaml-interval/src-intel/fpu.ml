...
let inf_pow y = if y < 0. then 0. else if y = 0. then 1. else infinity
...
let neg_inf_pow y =
  ...
  else if mod_float y 2. = 0. then infinity
  else neg_infinity

let pos_pow_inf x = if x < 1. then 0. else if x = 1. then 1. else infinity

let pos_pow_neg_inf x =
  if x < 1. then infinity else if x = 1. then 1. else 0.

external flog_pow : float -> float -> float = "flog_pow_caml" "flog_pow"
  [@@unboxed]

external flog_pow_low : float -> float -> float
  = "flog_pow_low_caml" "flog_pow_low"
  [@@unboxed]

external flog_pow_high : float -> float -> float
  = "flog_pow_high_caml" "flog_pow_high"
  [@@unboxed]
...
module Low = struct
  ...
  external pow : float -> float -> float = "flog_pow_low_caml" "flog_pow_low"
    [@@unboxed]
  ...
  external atan2 : float -> float -> float = "fatan2_low_caml" "fatan2_low"
    [@@unboxed]
...
module High = struct
  ...
  external atan2 : float -> float -> float = "fatan2_high_caml" "fatan2_high"
    [@@unboxed]
