...
module I = struct
  ...
  (* Redefine ( ** ) from Interval_base to keep the version of the original
     library. *)
  let ( ** ) ({low= a; high= b} : t) (n : int) : t =
    let open U in
    let nf = of_int n in
    let[@inline] pow_l x =
      if x = infinity then 0.
      else Low.pow x (if x < 1.0 then nf.high else nf.low)
    in
    let[@inline] pow_h x =
      if x = infinity then infinity
      else High.pow x (if x < 1.0 then nf.low else nf.high)
    in
    let sn = compare n 0 and sa = compare a 0. and sb = compare b 0. in
    if sn = 0 then
      if a = 0. && b = 0. then raise (Domain_error "**") else one
    else if sb < 0 then
      if n mod 2 = 0 then
      ...
      if sb = 0 then raise (Domain_error "**")
  ...
  let ( **. ) ({low= a; high= b} : t) (nf : fake_float) : t =
    ...
    let[@inline] pow_h x =
      if x = infinity then infinity else High.pow x nf
    in
  ...
  let mod_f ({low= a; high= b} : t) (y : float) : t =
  ...
  module Arr = struct
    ...
    let to_string_fmt fmt v =
      if U.(Array.length v = 0) then "[| |]"
        ...
        add_buffer b fmt v ; Buffer.contents b
...
let sprintf_I format i =
  Printf.sprintf "[%s, %s]"
    (Printf.sprintf format i.low)
    (Printf.sprintf format i.high)
...
let sinh_I (x : t) : t = I.sinh x

let tanh_I (x : t) : img = I.tanh x
...
let pow_I_I (x : t) (y : t) : t = x **$ y

let pow_I_f (x : t) (f : float) : t = x **$. f
