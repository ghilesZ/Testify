...
(** Implementation of interval functions that can be shared with the
    Interval_crlibm library. (Trigonometric functions depend on argument
    reduction which is performed differently.) *)

open Interval
module Low = Fpu.Low
module High = Fpu.High

let mod2 x = Fpu.fmod x 2.
...
let tan {low= a; high= b} =
  if -.max_63 <= a && b <= max_63 && Interval.High.(b -. a < pi) then
    ...
    if ta <= tb then {low= ta; high= tb} else Interval.I.entire
