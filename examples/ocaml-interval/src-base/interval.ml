(* Copyright 2011 Jean-Marc Alliot / Jean-Baptiste Gotteland Copyright 2018
   Christophe Troestler

   This file is part of the ocaml interval library.

   The ocaml interval library is free software: you can redistribute it
   ...
   along with the ocaml interval library. If not, see
...
(* [min] and [max], specialized to floats (faster). NaN do dot need to be
   handled (see [I.v]). *)
...
module L = struct
  ...
  external ( +. ) : float -> float -> float
    = "ocaml_low_add_byte" "ocaml_low_add"
    [@@unboxed]

  external ( -. ) : float -> float -> float
    = "ocaml_low_sub_byte" "ocaml_low_sub"
    [@@unboxed]

  external ( *. ) : float -> float -> float
    = "ocaml_low_mul_byte" "ocaml_low_mul"
    [@@unboxed]
...
module H = struct
  ...
  external ( +. ) : float -> float -> float
    = "ocaml_high_add_byte" "ocaml_high_add"
    [@@unboxed]
  ...
  external ( *. ) : float -> float -> float
    = "ocaml_high_mul_byte" "ocaml_high_mul"
    [@@unboxed]

  external ( /. ) : float -> float -> float
    = "ocaml_high_div_byte" "ocaml_high_div"
    [@@unboxed]
...
let low_cbr x = if x >= 0. then L.(x *. x *. x) else L.(x *. H.(x *. x))

let high_cbr x = if x >= 0. then H.(x *. x *. x) else H.(x *. L.(x *. x))

let rec low_pow_IN x n =
  (* x ∈ ℝ, n ≥ 0 *)
...
and low_pow_i x = function
  ...
  | 4 ->
      L.(
        ...
        x2 *. x2)
  | n ->
      if n >= 0 then low_pow_IN x n
      else
        (* Since the rounding has the same sign than xⁿ, we can treat u ↦ 1/u
           as decreasing. *)
        L.(1. /. high_pow_IN x (-n))
...
(* The [Low] and [High] modules below depend on both the previous [Low0] and
   [High0]. *)
module Low = struct
  include L
  ...
  let neg_pow_IN x = function
    ...
    | 4 ->
        ...
        x2 *. x2
    | n ->
        if is_even n then pos_pow_IN 1. (x *. x) (n / 2)
...
(* Invariants (enforced by [I.v]: - -∞ ≤ low ≤ high ≤ +∞. In particular, no
   bound is NaN. - [-∞,-∞] and [+∞,+∞] are not allowed. *)

module U = Interval__U (* Save original operators *)

let zero : t = {low= 0.; high= 0.}

let one : t = {low= 1.; high= 1.}

let entire : t = {low= neg_infinity; high= infinity}

let pi : t = {low= Low.pi; high= High.pi}

let two_pi : t = {low= Low.two_pi; high= High.two_pi}

let half_pi : t = {low= Low.half_pi; high= High.half_pi}

let e : t = {low= Low.e; high= High.e}

let v (a : float) (b : float) : t =
  if a < b (* ⇒ a, b not NaN; most frequent case *) then {low= a; high= b}
  else if a = b then
    ...
    (* a > b or one of them is NaN *)
    invalid_arg
      ( "Interval.I.v: [" ^ string_of_float a ^ ", " ^ string_of_float b
      ^ "] not allowed" )
...
let of_int (n : int) : t = {low= Low.float n; high= High.float n}
...
let pr ch i = Printf.fprintf ch "[%g, %g]" i.low i.high

let pp fmt i = Format.fprintf fmt "[%g, %g]" i.low i.high
...
let is_bounded {low; high} = neg_infinity < low && high < infinity

let is_entire {low; high} = neg_infinity = low && high = infinity

let equal {low= a; high= b} {low= c; high= d} = a = c && b = d

let subset {low= a; high= b} {low= c; high= d} =
  (* No empty intervals. *)
  c <= a && b <= d

let less {low= a; high= b} {low= c; high= d} = a <= c && b <= d
...
let width x : t = {low= Low.(x.high -. x.low); high= High.(x.high -. x.low)}
...
let mig x =
  if x.low >= 0. then x.low
  else if x.high <= 0. then -.x.high
  else (* x.low < 0 < x.high *) 0.

let abs ({low= a; high= b} as x : t) : t =
  if 0. <= a then x
  else if b <= 0. then {low= -.b; high= -.a}
  else {low= 0.; high= fmax (-.a) b}

let sgn ({low= a; high= b} : t) : t =
  {low= float (compare a 0.); high= float (compare b 0.)}

let truncate (x : t) : t = {low= floor x.low; high= ceil x.high}

let hull (x : t) (y : t) : t =
  {low= fmin x.low y.low; high= fmax x.high y.high}

let inter_exn ({low= a; high= b} : t) ({low= c; high= d} : t) =
  let low = fmax a c in
  let high = fmin b d in
  if low <= high then {low; high} else raise (Domain_error "I.inter_exn")

let inter ({low= a; high= b} : t) ({low= c; high= d} : t) : t option =
  let low = fmax a c in
  let high = fmin b d in
  if low <= high then Some {low; high} else None

let max (x : t) (y : t) : t =
  {low= fmax x.low y.low; high= fmax x.high y.high}
...
let ( ~- ) ({low= a; high= b} : t) : t = {low= -.b; high= -.a}
...
let ( /: ) (x : float) ({low= a; high= b} : t) : t =
  ...
  if sx = 0 then
    if sa = 0 && sb = 0 then raise Division_by_zero else {low= 0.; high= 0.}
...
let sqr ({low= a; high= b} : t) : t =
  if a >= 0. then {low= Low.(a *. a); high= High.(b *. b)}
  else if (* a < 0; a is not NaN *)
          b >= 0. then {low= 0.; high= fmax High.(a *. a) High.(b *. b)}
  else {low= Low.(b *. b); high= High.(a *. a)}

let cbr ({low= a; high= b} : t) : t = {low= Low.cbr a; high= High.cbr b}
...
let pow_IN x = function
  ...
  | n ->
      ...
      if is_even n then
        if x.low >= 0. then
          {low= Low.pos_pow_IN 1. x.low n; high= High.pos_pow_IN 1. x.high n}
        else if x.high > 0. then
          (* x.low < 0 < x.high *)
          { low= 0.
          ; high=
              fmax High.(neg_pow_IN x.low n) High.(pos_pow_IN 1. x.high n) }
        else
          (* x.low ≤ x.high ≤ 0 *)
...
(* let ( ** ) x n = if n >= 0 then pow_IN x n else inv (pow_IN x U.(-n)) *)
